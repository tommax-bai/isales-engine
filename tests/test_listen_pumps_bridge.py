"""change-1 Phase 1 (engine-eventbus-foundation §2) — listen-pump → EventBus →
bridge wiring.
...
wall-clock race), so the delivery guarantee is locked before change-2 inverts
the consumer.
"""
...
async def _connected_mock() -> tuple[MockTelephonyClient, object]:
...
async def test_lanes_registered_partial_lossy_final_lossless() -> None:
    ...
    tel, session = await _connected_mock()
    asr = ScriptedMockASR(partial_step_ms=10)
    providers = _make_providers(asr)
    ctx = await _start_listen_pumps(session, _make_config(), tel, providers)
    try:
        assert ctx.bus.lane_for(AsrPartial) is Lane.LOSSY
        assert ctx.bus.lane_for(AsrFinal) is Lane.LOSSLESS
    finally:
...
async def test_stop_pumps_drains_stream_ended_and_closes_bus() -> None:
    ...
    still-open bus; _stop_listen_pumps' aclose() drains the lossless lane →
    ...
    whole session before _stop_listen_pumps.)"""
    tel, session = await _connected_mock()
    asr = ScriptedMockASR(partial_step_ms=10)
    providers = _make_providers(asr)
    ctx = await _start_listen_pumps(session, _make_config(), tel, providers)
    ...
    assert ctx.bus._closed is True
