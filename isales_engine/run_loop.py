...
class Providers:
    ...
    asr: ASRProvider
    tts: TTSProvider
    # per-role-llm-config-and-restructure-card: per-slot LLM resolver. ``llm`` is
    # the single global default (fallback); ``llm_registry`` resolves each slot's
    # provider+model. Optional (None) so unit tests can build Providers without it
    # — call sites then fall back to the global ``llm``.
    llm_registry: LLMRegistry | None = None

    def llm_for(self, spec: object) -> LLMProvider:
        """Resolve the LLM client for a slot spec (or the global default)."""
        if self.llm_registry is not None:
            return self.llm_registry.resolve_spec(spec)
        return self.llm
...
async def _do_greeting(
    ...
    text = await generate_greeting(
        session,
        config.pipeline,
        # greeting is generated by the main slot → use its provider+model.
        providers.llm_for(config.pipeline.main),
